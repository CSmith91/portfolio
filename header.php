<!-- header.php -->
<head> 
    <meta charset="UTF-8"> 
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
    <title><?php echo $pageTitle; ?></title> 
    <link rel="stylesheet" href="./css/stylesheet.css">
    <link rel="stylesheet" href="./css/carousel.css">
</head> 
<header>
    <h1 id="home-link"><a href="./portfolio.php">Chris Smith's Portfolio</a></h1>
    <p><i>Note that the root of this domain is blank as it is sometimes used for building websites 'offline' while a pre-existing live website is running for less technical clients to view.</i></p>
            <nav>
                <ul>
                    <li><a href="./portfolio.php#skills">Skills</a></li>
                    <li><a href="./portfolio.php#projects">Projects</a></li>
                    <li><a href="./portfolio.php#education-certificates">Education & Certificates</a></li>
                    <li><a href="./portfolio.php#personal-projects">Personal Projects</a></li>
                    <li><a href="./portfolio.php#hobbies">Hobbies</a></li>
                    <li><a href="./contact.php">Contact Me</a></li>
                </ul>
            </nav>
</header>